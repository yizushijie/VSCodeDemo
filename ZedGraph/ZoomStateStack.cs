...
namespace ZedGraph
{
	...
	public class ZoomStateStack : CollectionPlus, ICloneable
	{
		/// <summary>
		/// Default Constructor
		/// </summary>
		...
		/// <summary>
		/// Public read-only indexer property for the <see cref="ZoomStateStack" />.
		/// </summary>
		public ZoomState this[int index]
		{
			get { return (ZoomState) List[index]; }
		}
		...
		public ZoomState Pop( GraphPane pane )
		{
			if ( !this.IsEmpty )
			{
				ZoomState state = (ZoomState) List[ this.Count - 1 ];
		...
		public ZoomState PopAll( GraphPane pane )
		{
			if ( !this.IsEmpty )
			{
				ZoomState state = (ZoomState) List[ 0 ];
				this.Clear();
		...
		public ZoomState Top
		{
			...
			{
				if ( !this.IsEmpty )
					return (ZoomState) List[ this.Count - 1 ];
				else
					return null;
			}
		}
